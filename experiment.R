#R version 3.4.4
#Occupancy analysis experimentation (runs ALL MODELS for ALL SPECIES) ((yep, it'll take *a while* to compute all this)) (((hopefully I fixed all the bugs ahead of time)))
...
#Import data (note: make sure to fix seasonList if you have different seasons)
...
#Preparing UMF for each ----
speciesList <- sort(unique(occ.dat$Species))
speciesList <- speciesList[!(speciesList %in% c("", "Unknown", "unknown"))]
allModels <- allLists <- list()

for (studySpecies in speciesList) {
  w <- which(speciesList == studySpecies)
  ...
  colextModelList <- function(varNames, data) {
    ...
    tic("Modeled EVERYTHING")
    #Preallocate some variables
    n <- nrow(fourMat)
    timeLeft <- as.list(rep(NA, 10)) #make it a list so the units for each entry are recorded separately
    cat("Building models...\n")
    startTime <- Sys.time()

    #Make all the models described by the rows of fourMat
    #WARNING: n is equal to length(combo.formChar)^4, so this loop will take a VERY VERY VERY LONG TIME to complete.
    outMap <- mapply(fourMat[,1],
                     ...
                     FUN = function(psi, gam, eps, p, i) {
                       ...
                       timeLeft[[(i %% 10) + 1]] <- (timeElapsed/completion) - timeElapsed
                       meanTimeLeft <- mean(unlist(timeLeft), na.rm = TRUE)
                       cat("\r", sprintf(fmt = "%.02f", 100*completion), "\b%",
                           "Estimate", signif(meanTimeLeft, digits = 3),
  ...
  #Function to produce a fitList based on a list of colext models and attach good names
  ...
  allModels[[w]] <- tryCatch(expr = {
    colextModelList(varNames = c("Site",
                                 "Height",
                                 "HumanInfluence",
                                 "WoodBiomass" #,
                                 # "survey",
                                 # "season"
                                 # These last two need some work. Since they're observation-level and season-level covariates, respectively, they're only valid in certain formula slots (p, and gamma/epsilon/p, respectively), and will throw errors if they're put in the wrong place (ruining the whole allModels[[w]], even if the others are legitimate models)
                                 # Consider something programmatic using something like
                                 # `grep(pattern = "*Covs", x = slotNames(umf), value = TRUE)`, may make the `varNames` argument unnecessary
    ),
    data = umf)
  },
  error = function(x) {as.character(geterrmessage())},
  warning = function(x) {as.character(warnings())})
  ...
  allLists[[w]] <- tryCatch(expr = {colextFitList(allModels[[w]])},
                            error = function(x) {as.character(geterrmessage())},
                            warning = function(x) {as.character(warnings())})

  allSels[[w]] <- tryCatch(expr = {modSel(allLists[[w]])},
                           error = function(x) {as.character(geterrmessage())},
                           warning = function(x) {as.character(warnings())})
...
#Save all the results to permanent files; use `load()` to read them back in later
save(allModels, file = "~/Documents/MooringModels-June2018.Rsave")
save(allLists, file = "~/Documents/MooringLists-June2018.Rsave")
save(allSels, file = "~/Documents/MooringSels-June2018.Rsave")

totalLength <- Sys.time() - veryBeginning
cat("\n\n -- Computations complete. This script took",
    round(totalLength, digits = 2),
    attr(totalLength, "units"),
    "to finish. Enjoy your results! -- \n\n")
